...
def LeNet():
    ...
    network.add(layers.ZeroPadding2D(padding=(1, 1), data_format=None))
    network.add(layers.Dropout(0.01))
    network.add(layers.ZeroPadding2D(padding=(1, 1), data_format=None))
    network.add(layers.Conv2D(filters=16, kernel_size=(3, 3), activation='relu'))
    network.add(layers.ZeroPadding2D(padding=(1, 1), data_format=None))
    network.add(layers.AveragePooling2D((2, 2)))
    network.add(layers.ZeroPadding2D(padding=(2, 2), data_format=None))
    network.add(layers.AveragePooling2D((2, 2)))
    network.add(layers.Dropout(0.01))
    network.add(layers.ZeroPadding2D(padding=(2, 2), data_format=None))
...
file = open("Docker-gpu-lnn.log", 'a+')
start1 = time.time()
network.fit(train_images, train_labels, epochs=10, batch_size=128, verbose=2)
file.write(str(time.time() - start1) + '\n' )
print(time.time() - start1)

start2 = time.time()
test_loss, test_accuracy = network.evaluate(test_images, test_labels)
print("test_loss:", test_loss, "    test_accuracy:", test_accuracy)
file.write(str(time.time() - start2) + '\n' )
file.write("test_loss:"+str(test_loss)+" test_accuracy:"+str(test_accuracy)+'\n')
file.close()
